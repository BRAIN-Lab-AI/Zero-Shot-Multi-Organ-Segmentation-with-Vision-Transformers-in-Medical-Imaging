...
class MedSAM25D(nn.Module):
    """MedSAM with learnable 2.5D fusion + FIXED LoRA"""
    def __init__(self, sam_model, num_slices=5, use_lora=True, lora_rank=4):
        super().__init__()
        ...
        # Add learnable fusion
        self.slice_fusion = SliceFusionModule(num_slices)
        
        # LoRA - FIXED
        if use_lora:
            self._inject_lora_fixed(lora_rank)
        # Freeze strategy
        for param in self.image_encoder.parameters():
            param.requires_grad = False
    
    def _inject_lora_fixed(self, rank):
        """FIXED: Properly inject LoRA into attention layers"""
        lora_count = 0
        ...
        print(f"✅ Injected LoRA into {lora_count} QKV layers")
        
        # Verify LoRA parameters are registered
        lora_params = sum(1 for n, p in self.named_parameters() if 'lora' in n.lower())
        print(f"✅ Total LoRA parameters registered: {lora_params}")
    
    def forward(self, multi_slice, box):
        # Fuse slices to 3-channel
...
def main():
    ...
    for epoch in range(args.num_epochs):
        ...
        if val_loader:
            ...
            if val_loss < best_loss:
                best_loss = val_loss
                save_path = os.path.join(args.work_dir, "MedSAM++_with_LoRA_best-frozen.pth")
                torch.save(model.state_dict(), save_path)
                print(f"  ✅ Best model saved! (LoRA params included)")
        else:
            val_losses.append(None)
            print(f"Epoch {epoch+1}: Train={train_loss:.4f}")
            torch.save(model.state_dict(), os.path.join(args.work_dir, "frozen-MedSAM++_with_LoRA_latest.pth"))
    
    # Verify saved checkpoint has LoRA
    saved_state = torch.load(os.path.join(args.work_dir, "MedSAM++_with_LoRA_best-frozen.pth" if val_loader else "frozen-MedSAM++_with_LoRA_latest.pth"))
    ...
    loss_curve_path = os.path.join(args.work_dir, 'frozen-medsam++_lora_fixed_loss_curve.png')
